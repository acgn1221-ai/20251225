...
let petConfigs = [ // 新增：寵物設定資料
  { name: "藍色氮氣飛天大象", frames: 7, w: 457, h: 49, path: '8-1/all.png', previewPath: '8-1/0.png', frameW: 0, frameH: 0 },
  { name: "究極炫酷紫色魔法獨角獸", frames: 8, w: 587, h: 92, path: '9-1/all.png', previewPath: '9-1/0.png', frameW: 0, frameH: 0 },
  { name: "豬豬", frames: 15, w: 970, h: 55, path: '10-1/all.png', previewPath: '10-1/0.png', frameW: 0, frameH: 0 }
];
...
function preload() {
  ...
  for (let i = 0; i < petConfigs.length; i++) {
    petPreviews[i] = loadImage(petConfigs[i].previewPath);
...
function drawPetSelection() {
  ...
  for (let i = 0; i < petConfigs.length; i++) {
    ...
    // 繪製預覽圖
    imageMode(CENTER);
    let img = petPreviews[i];
    image(img, x, y, img.width * 2, img.height * 2); // 放大預覽
